...
class Zhihu_Crawler(object):
	...
	def __update(self, collection, count):
		self.url = 'http://www.zhihu.com/collection/' + collection
		title = self.__get_collection_title()
		# print('    收藏夹 << ' + title + ' >>')
		page_index = 1
		page = ''
	...
	def update_collections(self, cookies):
		session = requests.session()
		html = session.get('http://www.zhihu.com/collections', \
			cookies = cookies)
		text = bs(html.text, 'html.parser')
...
if cookies != None:
	spider.update_collections(cookies)
else:
	sys.exit()
